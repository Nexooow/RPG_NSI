...
with open("meteor_sprite_sheet.txt","r") as f:
    b64=f.read()
class Meteor:
    def __init__(self,center,image=b64,frame_index=0,object_density=3000.0):
        ...
        self.mass=self.meteor_density*(4.0/3.0)*math.pi*(self.radius**3) #on assume que l'objet est une sphère
        self.term_v=math.sqrt((2*self.mass*self.g)/(self.air_density*self.Cd*self.surface))
    def deplace(self):
        v=self.speed
    ...
    def impact_force(self):
        return int(0.5*self.mass*(self.x_speed**2+self.speed/self.mps*self.dt)/self.point_of_fall)
